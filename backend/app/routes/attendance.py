"""
ATTENDANCE.PY - FICHAS DE ATENDIMENTO
======================================
"""
...
from app.models import (
    User, AttendanceRecord, AttendanceRecordCreate, AttendanceRecordResponse,
    StrandTest, StrandTestCreate, StrandTestResponse
)
...
async def create_attendance_record(
    ...
    if not appointment or appointment["profissional_id"] != current_user.id:
        raise HTTPException(403, "Agendamento inválido")
...
async def update_attendance_record(
    ...
    if current_user.id == record["cliente_id"]:
        if "feedback" not in update_data:
            raise HTTPException(400, "Cliente só pode atualizar feedback")
...
async def upload_attendance_photos(
    record_id: str,  # Path parameter - obrigatório
    tipo: str,  # Query/Body parameter - obrigatório
    current_user: Annotated[User, Depends(get_current_user)],  # Dependency - obrigatório
    files: List[UploadFile] = File(...)  # Com default (File(...)) - opcional
):
    """
    Upload de fotos do atendimento (profissional)
    
    Args:
        record_id: ID da ficha de atendimento (path parameter)
        tipo: "antes" ou "depois" (query/body parameter)
        current_user: Usuário autenticado (dependency)
    ...
    return {"message": "Fotos enviadas", "urls": photo_urls}
